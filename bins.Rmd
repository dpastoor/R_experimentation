...
Given a traditional PK dataset that looks like:
...
This can be similarly accomplished with binning
...
```{r}
 binned_data %>% mutate(WTBIN2 = set_bins(WEIGHT, breaks = c(0, 50, 70), quiet=F)) %>% head
...
sid_data %>% mutate(AGECUTS = cut(AGE, breaks = quantile(AGE), include.lowest=T),
                    AGEBINS = as.numeric(AGECUTS)) %>% head
...
```{r}
sid_data %>% mutate(AGEBINS = set_bins(AGE)) %>% head
```
...
```{r}
tw_data %>% group_by(TW) %>% summarize(n= n())
